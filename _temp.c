...
static void OLED_SetPageCol(uint8_t page, uint8_t x){
...
static void show_raw_line(int raw){
    ...
    // Clear top line only
    uint8_t zeros[128]={0};
    OLED_SetPageCol(0,0); I2C1_DataBurst(zeros,128);
    OLED_PrintAt(0,0,line);
}

static int detected_stable_active_low(void){
    // FC-51 typical: active LOW = object
    int votes=0;
    for(int i=0;i<8;i++){ if(sensor_raw()==0) votes++; delay_cycles(80000); }
    return (votes>=6);
}
